...
NonDiffusePointOctree* NonDiffusePointOctreeCache::find(const std::string& fileName)
{
    ...
    if(i == m_cache.end())
    {
    	...
    	NonDiffusePointArray pointArray;
    	NonDiffusePointOctree* pointsOctree;
    	if(loadNonDiffusePointFile(pointArray, fileName)) {
    		pointsOctree = new NonDiffusePointOctree(pointArray);
